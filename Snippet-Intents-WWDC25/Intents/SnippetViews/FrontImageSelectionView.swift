    ...
    var body: some View {
        VStack(spacing: 20) {
            ImageSelectionHeader(
                title: "Add Front Image", systemImage: "photo.badge.plus",color: .green
            )
            ...
            HStack(spacing: 12) {
                Button(intent: SelectRandomImageIntent(postcard: postcard.toEntity, surface: .front)) {
                    ActionButtonLabel(title: "Generate", style: .primary(.green))
                }
                .buttonStyle(.plain)
                
                Button(intent: SelectCustomImageImageIntent(postcard: postcard.toEntity, surface: .front)) {
